...
public class CarSecureService extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(domain.equals("getDetailItem")){
            JSONObject obj = JSONObject.fromObject(params);
            String sdid=obj.get("sdid").toString();
            String sql="select id,content,url,logopic,title,coverpic from wx_article where UPPERID='"+sdid+"'";
            ui = new UserHttpImpl();
            jsonStr = ui.queryAnyList(getJsonSql("queryAnyListSQL",sql));
        }
        ...
        if(domain.equals("cusEdit")){
            ...
            String usertype=obj.get("usertype").toString();
            String sql="UPDATE STORE_USER_DETAILS SET MANAGER_ID='"+managerid+"',D_NAME='"+name+"',D_PROVINCE='"+province+"',D_VIN='"+vin+"',D_ENGINENUM='"+enginenum+"',D_PLATENUM='"+platenum+"',D_CITY='"+city+"',D_STARTDATE='"+startdate+"',D_ENDDATE= '"+enddate+"',D_USERTYPE= '"+usertype+"' WHERE D_DETAILID='"+userid+"'";
        ...
        if(domain.equals("getMangerList")){
            ...
            String sql="";
            if(StringUtil.isBlank(openid)){
                sql="SELECT D_DETAILID,U_USERID,D_NAME,D_GENDER,D_BIRTHDAY,D_PROVINCE,D_CITY,D_BRANCH,D_ADDRESS,D_PLATENUM,D_VIN,D_DRIVERLICENCE,D_ENGINENUM,D_STARTDATE,D_ENDDATE,MANAGER_ID,D_TYPE,D_CODEPIC,D_MOBILE,D_REMINDTIME FROM STORE_USER_DETAILS WHERE D_TYPE=1";
            } else{
                sql="SELECT D_DETAILID,U_USERID,D_NAME,D_GENDER,D_BIRTHDAY,D_PROVINCE,D_CITY,D_BRANCH,D_ADDRESS,D_PLATENUM,D_VIN,D_DRIVERLICENCE,D_ENGINENUM,D_STARTDATE,D_ENDDATE,MANAGER_ID,D_TYPE,D_CODEPIC,D_MOBILE,D_REMINDTIME FROM STORE_USER_DETAILS WHERE U_USERID IN (SELECT MANAGER_ID FROM STORE_USER_DETAILS WHERE U_USERID IN (SELECT U_USERID FROM STORE_USER_INFO  WHERE OPENID='"+openid+"'))";
            }
            jsonStr = ui.queryAnyList(getJsonSql("queryAnyListSQL",sql));
        }
        if(domain.equals("CheckReg")){
            ...
            sb.append("SELECT D_DETAILID,U_USERID,D_NAME,D_GENDER,D_BIRTHDAY,D_PROVINCE,D_CITY,D_BRANCH,D_ADDRESS,D_PLATENUM,D_VIN,D_DRIVERLICENCE,D_ENGINENUM,D_STARTDATE,D_ENDDATE,MANAGER_ID,D_TYPE,D_CODEPIC,D_MOBILE,D_REMINDTIME FROM STORE_USER_DETAILS WHERE U_USERID IN (SELECT U_USERID FROM STORE_USER_INFO WHERE 1=1 ");
        ...
        if(domain.equals("getUser")){
            ...
            ui = new UserHttpImpl();
            StringBuffer sql=new StringBuffer();
            sql.append("SELECT D_DETAILID,U_USERID,D_NAME,D_GENDER,D_BIRTHDAY,D_PROVINCE,D_CITY,D_BRANCH,D_ADDRESS,D_PLATENUM,D_VIN,D_DRIVERLICENCE,D_ENGINENUM,D_STARTDATE,D_ENDDATE,MANAGER_ID,D_TYPE,D_CODEPIC,D_MOBILE,D_REMINDTIME FROM STORE_USER_DETAILS WHERE D_DETAILID='"+userid+"'");
            jsonStr = ui.queryAnyList(getJsonSql("queryAnyListSQL",sql.toString()));
        }
